...
/// Graphics
    ...
    void Graphics::DrawMesh(Ref<Mesh> mesh1, MeshData& mesh, Vector4 position, Vector4 rotation, Material& material, Ref<Camera> camera) {
        if (graphicsImpl) {
            graphicsImpl->DrawMeshImpl(mesh1, mesh, position, rotation, material, camera);
        }
    }
    ...
    void Graphics::RenderMesh(Ref<Mesh> mesh1, MeshData& mesh, Matrix4x4 transform, Material& material, Ref<Camera> camera) {
        if (graphicsImpl) {
            graphicsImpl->RenderMeshImpl(mesh1, mesh, transform, material, camera);
        }
    }
