...
enum class RigidBodyConstantKeys {
    ID, Mass, Drag, AngularDrag, UseGravity, BodyType
};

static std::map<RigidBodyConstantKeys, const char*> rigidBodyKeys = {
    { RigidBodyConstantKeys::ID, "ID" },
...
namespace YAML {
    template<>
    struct convert<VWolf::RigidBodyComponent>
    {
        static bool decode(const Node& node, VWolf::RigidBodyComponent& rhs)
        {
            if (!node.IsMap())
                return false;
            
            if (node[rigidBodyKeys[RigidBodyConstantKeys::ID]]) {
                rhs.SetID(node[rigidBodyKeys[RigidBodyConstantKeys::ID]].as<VWolf::UUID>());
            }
...
namespace VWolf {
    YAML::Emitter& operator<<(YAML::Emitter& out, VWolf::RigidBodyComponent& v)
    {
        out << YAML::BeginMap;
        out << YAML::Key << rigidBodyKeys[RigidBodyConstantKeys::ID] << YAML::Value << v.GetID();
